...
def define_arguments(parser):
    parser.set_defaults(func=encoding)
...
def alpha_to_int(data, zero_based=False, overflow=False):
    ...
    """
    return list(
        map(lambda c: alpha_to_int_single(c, zero_based, overflow), data))
...
def int_to_alpha(sequence, zero_based=False, overflow=False):
    ...
    """
    return list(
        map(lambda i: int_to_alpha_single(i, zero_based, overflow), sequence))
...
def encoding(data, args):
    ...
    if types[args.input] == "a" and types[args.output] == "i":
        encoding = list(
            map(str, alpha_to_int(data, args.zero_based, args.overflow)))
